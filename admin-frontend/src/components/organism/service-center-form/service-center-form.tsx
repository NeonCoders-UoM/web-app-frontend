...
export default function ServiceCenterForm({ initialData, onSubmit }: ServiceCenterFormProps) {
  ...
  const availableServiceOptions = [
    "Oil Change", "Tire Replacement", "Brake Service", "Engine Repair", "Full Inspection"
  ];
  ...
  const validateForm = (): boolean => {
    ...
    const { serviceCenterName, email, address, telephoneNumber, ownersName, vatNumber, registrationNumber, commissionRate, availableServices } = formData;

    if (!serviceCenterName.trim()) newErrors.serviceCenterName = "Service Center Name is required";
    if (!email.trim()) newErrors.email = "Email is required";
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) newErrors.email = "Invalid email format";
    if (!address.trim()) newErrors.address = "Address is required";
    if (!telephoneNumber.trim()) newErrors.telephoneNumber = "Telephone Number is required";
    else if (!/^\d{10}$/.test(telephoneNumber)) newErrors.telephoneNumber = "Telephone Number must be 10 digits";
    ...
    if (!commissionRate.trim() || isNaN(Number(commissionRate))) newErrors.commissionRate = "Commission Rate must be a number";
    if (availableServices.length === 0) newErrors.availableServices = "Please select at least one service";
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-2 gap-x-6 gap-y-4">
          {[
            ...
            { id: "commissionRate", label: "Commission Rate (%)", type: "number" }
          ].map(({ id, label, type = "text" }) => (
            <div key={id}>
              <label htmlFor={id} className="block mb-1" style={{ color: colors.neutral[600] }}>{label}</label>
              <InputField
                id={id}
                name={id}
                type={type}
                placeholder={label}
                value={formData[id as keyof Omit<ServiceCenter, "id">] as string}
